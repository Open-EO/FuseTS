...
def generate_peakvalley_udp():
    ...
    drop_param = Parameter.number(
        name="drop_thr", description="Threshold value for the amplitude of the drop in the input feature", default=0.15
    )
    ...
    slope_param = Parameter.number(
        name="slope_thr", description="Threshold value for the slope where the peak should start", default=-0.007
    )

    context = {
        "drop_thr": {"from_parameter": "drop_thr"},
        "rec_r": {"from_parameter": "rec_r"},
        "slope_thr": {"from_parameter": "slope_thr"},
    }
    process = apply_dimension(
        input_cube,
        process=lambda x: run_udf(x, udf=load_peakvalley_udf(), runtime="Python", context=context),
        dimension="t",
    )
    ...
    return publish_service(
        ...
        parameters=[input_cube.to_dict(), drop_param.to_dict(), rec_param.to_dict(), slope_param.to_dict()],
