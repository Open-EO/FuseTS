...
def plot_whittaker(mean_ndvi):
    ...
    plt.plot(rye_ndvi.t.sel(t=date_range), smooth_optim.sel(t=date_range), '-', label='Whittaker S2 NDVI lambda optimized')
    plt.ylabel ('NDVI')
    plt.grid(True)
    plt.legend()
...
def plot_pipeline():
    ...
    plt.plot(rye_ndvi.sel(t=date_range).t, rye_ndvi.isel(x=middle_x, y=middle_y).sel(t=date_range), 'o',
             label='Masked NDVI')
    plt.plot(rye_unmasked_ndvi.sel(t=date_range).t, rye_unmasked_ndvi.isel(x=middle_x, y=middle_y).sel(t=date_range), '-',
             label='Unmasked NDVI',linewidth='2')
    ...
    plt.plot(ndvi_series.t, ndvi_series, '-',
             label='Masked NDVI',linewidth='2')
    plt.plot(rye_unmasked_ndvi.sel(t=date_range).t, rye_unmasked_ndvi.isel(x=middle_x, y=middle_y).sel(t=date_range), 'o',
             label='Unmasked NDVI')
    ax.xaxis.set_major_formatter(date_form)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ...
    plt.plot(rye_ndvi.sel(t=date_range).t, rye_ndvi.isel(x=middle_x, y=middle_y).sel(t=date_range), 'o',
             label='Masked NDVI')
    plt.plot(smooth100.sel(t=date_range).t, smooth100.isel(x=middle_x, y=middle_y).sel(t=date_range),
             '-',
             label='smooth NDVI',linewidth='2')
    ...
    plt.plot(rye_ndvi.sel(t=date_range).t, rye_ndvi.isel(x=middle_x, y=middle_y).sel(t=date_range), 'o',
             label='Masked NDVI')
    plt.plot(ds['RVI'].sel(t=date_range).t, ds['RVI'].isel(x=middle_x, y=middle_y).sel(t=date_range), '.',
             label='RVI')
    plt.plot(rye_ndvi_mogpr.NDVI.sel(t=date_range).t, rye_ndvi_mogpr.NDVI.isel(x=middle_x, y=middle_y).sel(t=date_range), '-',
             label='Fused NDVI',linewidth='2')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    hide_axis_labels()
    ...
    fig, ax = plt.subplots(figsize=size)
    ax.set_ylim([ymin, ymax])
    plt.plot(rye_ndvi_mogpr.NDVI.sel(t=date_range).t,
